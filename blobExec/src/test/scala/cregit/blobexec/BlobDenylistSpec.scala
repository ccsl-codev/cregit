...
class BlobDenylistSpec extends AnyFunSuite with Matchers {
  ...
  // The four blobs, and the sizes `git cat-file --batch-check` reports for them in
  // tencent__tencentkona-21's bare clone. All four are historical versions of one
  // OpenJDK langtools regression test, at two paths after a repository
  // reorganisation — which is why the list is keyed on content, not on path.
