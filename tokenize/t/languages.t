...
# Holds every consumer to CregitLanguages: both gates agree on the language, the
# parsers exist, the mask matches the table both ways, and srcml really parses
# each extension.
...
for my $dead (qw(go md yaml)) {
    ok(!exists $EXT_LANG{$dead}, "[$dead] is not in the table — cregit has no parser for it");
}
...
for my $ext (sort keys %EXT_LANG) {
    ...
    # srcml exits 0 and emits no <unit> for an extension it does not know, so the
    # exit status alone proves nothing. Only output does.
    ok(length($out) > 0,
