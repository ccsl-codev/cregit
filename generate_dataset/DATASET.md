...
| a blob JGit will not materialise that nothing identifies as generated | `blobsUntokenizable`, plus one `UNTOKENIZABLE blob` line each | no rows for those blobs — and unlike every other row in this table, the exclusion is **not explained**: on the evidence it is a large hand-written file. Reported, not gated, so the run still exits 0 and this counter plus that line are the only record the path is missing. Resolve it by confirming the file is generated (a header marker or a denylist entry with a citation), or by making it tokenizable |
