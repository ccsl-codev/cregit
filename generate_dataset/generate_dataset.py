...
{meta_sql}                t.file_path,
            ...
            LEFT JOIN (
                ...
                FROM footers f
                -- regexp_extract returns '' rather than NULL on no match, so
                -- without nullif a no-address trailer joins on '' and matches
                -- every `Name <>` row. Bracketed form first: correct precedence.
                LEFT JOIN emails fe
